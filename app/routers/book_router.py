...
@router.post("/books")
async def create_book(name: str, author: str, release_year: int, book_dal: BookLoader = Depends(get_book_dal)):
...
async def update_book(book_id: int, name: Optional[str] = None, author: Optional[str] = None, release_year: Optional[int] = None,
                      book_dal: BookLoader = Depends(get_book_dal)):
    return await book_dal.update_book(book_id, name, author, release_year)


@router.get("/books")
async def get_all_books(book_dal: BookLoader = Depends(get_book_dal)) -> List[Book]:
